...
public class AudioManagerEditor : Editor
{
    ...
    public override void OnInspectorGUI ()
    {
        ...
        var style_foldAudio = GUILayout.MaxWidth((EditorGUIUtility.currentViewWidth - 60) / 2f);
        var style_centeredHeader = new GUIStyle(EditorStyles.boldLabel) {alignment = TextAnchor.MiddleCenter, fontSize = 16 };

        if (_eventsFoldout = EditorGUILayout.Foldout(_eventsFoldout, "Events", true)) {
        ...
        if (_audioFoldout = EditorGUILayout.Foldout(_audioFoldout, "FMOD Events", true)) {
            using (new EditorGUILayout.HorizontalScope()) {
